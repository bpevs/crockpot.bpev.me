...
(function (mod) {
  ...
  */
    ...
    function tokenBase(stream, state) {
      ...
      if (
        hooks.tokenBase && (style = hooks.tokenBase(stream, state)) != false
      ) {
        return style;
      }
    ...
    return {
      ...
      indent: function (state, textAfter) {
        ...
        if (ctx.type == "statement") {
          ...
          closingBracket.test(ctx.type) && ctx.align && !dontAlignCalls
        ) {
  ...
  CodeMirror.defineMIME("text/x-tlv", {
    ...
    hooks: {
      "\\": function (stream, state) {
        ...
        if (
          (stream.sol()) &&
          ((/\\SV/.test(stream.string)) || (/\\TLV/.test(stream.string)))
        ) {
          ...
          if (
            (/\\TLV/.test(curPunc) && !state.vxCodeActive) ||
            (curPunc == "\\TLV_version" && state.vxCodeActive)
          ) {
            state.vxCodeActive = true;
          }
          style = "keyword";
          state.tlvCurCtlFlowChar = state.tlvPrevPrevCtlFlowChar = state
            .tlvPrevCtlFlowChar = "";
      ...
      token: function (stream, state) {
        if (
          state.vxCodeActive == true && stream.sol() &&
          state.tlvCurCtlFlowChar != ""
        ) {
