  ...
  CodeMirror.defineMode("stylus", function (config) {
    ...
    states.block = function (type, stream, state) {
      ...
      if (type == "variable-name") {
        if (
          (stream.indentation() == 0 && startOfLine(stream)) ||
      ...
      if (type && type.charAt(0) == "@") {
        // Property Lookup
        if (
          stream.indentation() > 0 && wordIsProperty(stream.current().slice(1))
        ) {
          override = "variable-2";
          return "block";
        }
      ...
      indent: function (state, textAfter, line) {
        ...
        if (
          ...
          if (
            ...
            /(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(ch) || wordIsTag(lineFirstWord)
          ) {
