  ...
  CodeMirror.defineMode("markdown", function (cmCfg, modeCfg) {
    ...
    function blockNormal(stream, state) {
      ...
      if (state.indentationDiff >= 4) {
        ...
        state.prevLineHasContent && !state.quote && !prevLineIsList &&
        ...
        modeCfg.fencedCodeBlocks && stream.match(/^```[ \t]*([\w+#]*)/, true)
      ) {
        // try switching mode
    ...
    function htmlBlock(stream, state) {
      ...
      if (
        (htmlFound && state.htmlState.tagStart === null &&
    ...
    function inlineNormal(stream, state) {
      ...
      if (
        ch === "]" && state.linkText && stream.match(/\(.*\)| ?\[.*\]/, false)
      ) {
        if (modeCfg.highlightFormatting) state.formatting = "link";
        var type = getType(state);
      ...
      if (!modeCfg.underscoresBreakWords) {
        if (
          ch === "_" && stream.peek() !== "_" && stream.match(/(\w)/, false)
        ) {
    ...
    var mode = {
      ...
      token: function (stream, state) {
        ...
        if (stream.sol()) {
          ...
          state.f = state.block;
          var indentation =
            stream.match(/^\s*/, true)[0].replace(/\t/g, "    ").length;
          var difference = Math.floor((indentation - state.indentation) / 4) *
            4;
