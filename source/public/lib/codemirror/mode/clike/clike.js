...
(function (mod) {
  ...
  CodeMirror.defineMode("clike", function (config, parserConfig) {
    ...
    function pushContext(state, col, type) {
      ...
      if (
        state.context && isStatement(state.context.type) && !isStatement(type)
      ) {
    ...
    function typeBefore(stream, state) {
      ...
      if (
        /\S(?:[^- ]>|[*\]])\s*$|\*$/.test(stream.string.slice(0, stream.start))
      ) {
        return true;
      }
    }
    ...
    return {
      ...
      token: function (stream, state) {
        ...
        if ((curPunc == ";" || curPunc == ":" || curPunc == ",")) {
          ...
          var type = "statement";
          if (
            curPunc == "newstatement" && indentSwitch &&
            stream.current() == "switch"
          ) {
